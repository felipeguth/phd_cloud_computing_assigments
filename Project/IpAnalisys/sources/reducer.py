...
totConnServ = sum(IPcount.values())
print '%s\t%d' % ("REDUCER-out1 COUNT NUMBER OF CONNECTIONS TO THE SERVER:", totConnServ)


#Number of entries for each IP
print("REDUCER-out2 NUMBER OF ENTRIES FOR EACH IP:")
for ip in IPcount.keys():
    print '%s\t%s'% ( ip, IPcount[ip])

#List of distinct IPs
print("REDUCER-out3 LIST OF DISTINCT IPs:")
for ip in IPcount.keys():
    print '%s\t'% ( ip)
